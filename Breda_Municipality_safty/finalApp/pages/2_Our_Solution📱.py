...
def get_dictionary_for_model(f_path = r'C:\Users\domin\Desktop\Team-based Working\Deliverables\data\data_for_model.csv'):
    ...
    name_date_crime_nhood_dfs = {
        ...
        n_name: pd.DataFrame({
            'ds': [date for date in df[df.neighbourhood_name == n_name].drop(columns='neighbourhood_name').rename(columns = {
            'date':'ds',
            'number_of_crimes': 'y'
        }).ds.unique()],
            ...
            'date':'ds',
            'number_of_crimes': 'y'
        }).ds == date]['y']) for date in df[df.neighbourhood_name == n_name].drop(columns='neighbourhood_name').rename(columns = {
            'date':'ds',
            'number_of_crimes': 'y'
        }).ds.unique()]
...
def create_models_for_neighbourhoods(crimes_dictionary,f_path = r'C:\Users\domin\Desktop\Team-based Working\Deliverables\data\data_for_model.csv'):
    ...
    models_per_n = {
        ...
        n_name: Prophet(changepoint_prior_scale = 0.001, seasonality_prior_scale=0.01,seasonality_mode='multiplicative').fit(crimes_dictionary[n_name]) for n_name in df.neighbourhood_name.unique()
                ...
                if options and options != []:
                    ...
                    with col2:
                        ...
                        predictions_dictionary = {
                            ...
                            'predicted_number_of_crimes':[prediction['predicted_number_of_crimes'][0] for name, prediction in predictions.items()]
                        }
...
with tab3:
    ...
    if st.button('Data Quality Report'):
        webbrowser.open('https://edubuas-my.sharepoint.com/:b:/g/personal/223834_buas_nl/EQMoBQDK9XdMpMObu-o_FrsBjeV6Xgv1CMoUJZ5YZSghqQ?e=aOLdUE')

    if st.button('Ethics Report'):
        webbrowser.open('https://edubuas-my.sharepoint.com/:w:/g/personal/224215_buas_nl/EXKE-ZuIFKBBjqh94BRRm8IBCr51o5L75S73NvEtztcTsA?e=ezPE9X')
