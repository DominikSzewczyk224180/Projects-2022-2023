...
tab1, tab2, tab3 = st.tabs(["Trend of crimes & Impact of nuisances", "Crime Rate vs Region in Breda", "Crime Rate vs Season"])
...
with tab1:
    ...
    recorded_crimes_Breda_per_month = recorded_crimes_Breda_per_month.rename(columns={'Geregistreerde misdrijven (aantal)' : 'Registered Crimes (total)'})
    ColumnsTokeep = ['Type of Crime', 'Periods' ,'Districts and neighbourhoods', 'Registered Crimes (total)' ]
    TotalCrimes = recorded_crimes_Breda_per_month.drop(columns=[col for col in recorded_crimes_Breda_per_month.columns if col not in ColumnsTokeep])
    ...
    def main():
        #show_map = st.button("Show Neighborhood Analysis")
            ...
            # Filtering on range
            year_range = st.slider('Select Year Range', int(CrimeVsQol['date'].dt.year.min()), int(CrimeVsQol['date'].dt.year.max()), (int(CrimeVsQol['date'].dt.year.min()), int(CrimeVsQol['date'].dt.year.max())))
            year_range = [pd.to_datetime(str(year)) for year in year_range]

            # Creating filters
            filtered_data = CrimeVsQol[CrimeVsQol['neighbourhood_name'].isin(selected_neighborhoods) & CrimeVsQol['date'].between(year_range[0], year_range[1]) & (CrimeVsQol['number_of_crimes'] > 0)]
...
with tab2:
    ...
    crime_names = ['abuse', 'accidents', 'arms_trade', 'drug_trafficking', 'human_trafficking', 'murders', 'open_violence', 'people_smuggling', 'pick_pocketing', 'robbery', 'shoplifting', 'street_robbery']

    number_of_crimes_per_type = []

    for crime_t in [abuse, accidents, arms_trade, drug_trafficking, human_trafficking, murders, open_violence, people_smuggling, pick_pocketing, robbery, shoplifting, street_robbery, threat]:
    ...
    fig.add_trace(go.Bar(x=list(crime_name_num_dict.keys())[0:3], y=list(crime_name_num_dict.values())[0:3]), row=1, col=1)
    fig.add_trace(go.Bar(x=list(crime_name_num_dict.keys())[3:7], y=list(crime_name_num_dict.values())[3:7]), row=1, col=2)
    fig.add_trace(go.Bar(x=list(crime_name_num_dict.keys())[7:], y=list(crime_name_num_dict.values())[7:]), row=1, col=3)
    ...
    st.markdown("Explore the spatial distribution of registered nuisances from 2018 to 2022. Gain insights into evolving trends and patterns, offering a comprehensive view of the city's nuisance landscape over the years.")
    ...
    selected_years = st.slider('Select years', min_value=min(available_years), max_value=max(available_years), value=(min(available_years), max(available_years)))
    ...
    st.write('Upon reviewing registrations from Breda, it becomes apparent that there is a significant variation in crime and nuisance rates among different neighborhoods. After identifying the ten neighborhoods with the highest crime rates in Breda, we further examined the occurrence of nuisances. We observed that regions experiencing high rates of crime also tend to have high levels of nuisances. This implies that an increase in the number of nuisances can sometimes lead to an uptick in crime. Additionally, we explored the correlation between the Quality of Life index and crime rate in each neighborhood. It was observed that neighborhoods with higher quality of life tend to be less susceptible to crime. Consequently, it is of utmost importance for law enforcement to assess the conditions in different neighborhoods, drawing on historical data, in order to allocate resources in a suitable and efficient manner.')
...
with tab3:
    ...
    def create_graph(df):
        ...
        fig.add_trace(go.Scatter(x=df['Periods'], y=df['Registered Crimes (total)'],mode='lines', name='Registered Crimes'))
        ...
        fig.add_trace(go.Scatter(x=df['Periods'], y=trend_line, mode='lines', name='Trend Line', line=dict(color='red', dash='dash')))
    ...
    df = recorded_nuisances_Breda_per_year[recorded_nuisances_Breda_per_year['Overlast'] == 'Total nuisance registrations']
    ...
    fig.add_trace(go.Scatter(x=df.index, y=trend_line, mode='lines', name='Trend Line', line=dict(color='red', dash='dash')))
    ...
    st.write("While exploring data about the number of nuisances registered per month we expected the number of nuisances to decrease as the number of crimes decreased. In fact, the number of nuisances increased by 300 since 2012.")

    st.subheader("Influence of the season on crimes and nuisances")

    st.write("After analysing both nuisances & crimes we can see that there are several spikes throughout each year of observation. We came up with a conclusion that these spikes are related to different seasons of the year and depending on the season of the year crimes probability can be higher or lower.")
    ...
    recorded_crimes_Breda_per_month_for_plot = recorded_crimes_Breda_per_month_for_plot[recorded_crimes_Breda_per_month_for_plot["Breda"] == "Breda"]

    recorded_crimes_Breda_per_month_for_plot = recorded_crimes_Breda_per_month_for_plot[recorded_crimes_Breda_per_month_for_plot["Totaal misdrijven"] == "Totaal misdrijven"]

    recorded_crimes_Breda_per_month_for_plot = recorded_crimes_Breda_per_month_for_plot.drop(['Totaal misdrijven', 'Breda'], axis=1)
    ...
    for i in [12, 24, 36, 48, 60, 72, 84, 96, 108, 120]:
        ...
        fig.add_trace(go.Scatter(x=x, y=y, mode='lines+markers', marker=dict(color=color[year_num]), name=years[year_num]))
        year_num += 1
    ...
    st.markdown("- Over the years, a consistent trend of increasing and decreasing nuisances can be observed during the summer and winter months respectively. January and February are notably safer compared to June and July, as they exhibit the highest occurrence of nuisances.")
    st.subheader("Crimes:")
    st.markdown("- While it is challenging to establish a definite pattern for crimes, it is evident that March, August, and December are the least crime-prone months, whereas July, November, and February experience the highest crime rates.")
    st.subheader("Considering our findings, it is evident that the season of the year significantly influences the rates of crime and nuisances. This information should be taken into account when determining the necessary resources to ensure city safety.")
